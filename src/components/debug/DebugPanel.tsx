...
const MINIGAME_TYPES: MiniGameType[] = [
  'breathing-rhythm', 'struggle-button', 'hold-to-delete', 'word-puzzle',
  'quick-time-event', 'hidden-object', 'find-the-difference', 'planting-game',
  'drag-drop-scheduler', 'room-decoration', 'slider-puzzle',
]
...
const SAMPLE_CONFIGS: Record<MiniGameType, unknown> = {
  ...
  'hold-to-delete':      { holdDuration: 1200, elements: [{ id: 'a', label: 'Foto berdua', color: '#c9a0dc' }, { id: 'b', label: 'Chat lama', color: '#7ec8a0' }] },
  'word-puzzle':         { target: 'IKHLAS', letters: ['I','K','H','L','A','S','E','O'], timeLimit: 30 },
  'quick-time-event':    { duration: 15, spawnInterval: 1.2, targets: Array.from({ length: 12 }, (_, i) => ({ id: 't' + String(i), x: 10 + Math.random() * 80, y: 10 + Math.random() * 80, label: '!' })) },
  'hidden-object':       { timeLimit: 30, targets: [{ id: 'o1', x: 35, y: 45, radius: 8, label: 'Foto' }, { id: 'o2', x: 65, y: 30, radius: 8, label: 'Surat' }] },
  'find-the-difference': { timeLimit: 45, differences: [{ id: 'd1', x: 40, y: 50, radius: 10 }, { id: 'd2', x: 70, y: 30, radius: 10 }] },
  'planting-game':       { steps: [{ id: 's1', label: 'Siapkan pot', icon: '🪴' }, { id: 's2', label: 'Isi tanah', icon: '🌱' }, { id: 's3', label: 'Tanam', icon: '🌵' }, { id: 's4', label: 'Siram', icon: '💧' }], correctOrder: ['s1','s2','s3','s4'] },
  'drag-drop-scheduler': { tasks: [{ id: 't1', label: 'Kuliah PBO', duration: 1, color: '#7ec8a0' }, { id: 't2', label: 'Makan siang', duration: 1, color: '#c87941' }], slots: ['08:00','10:00','12:00'], correctMapping: { t1: '08:00', t2: '12:00' } },
  'room-decoration':     { items: [{ id: 'kaktus', label: 'Kaktus', color: '#3a8a5f' }, { id: 'buku', label: 'Buku', color: '#c87941' }], slots: [{ id: 'meja', label: 'Meja', x: 30, y: 40, accepts: 'buku' }, { id: 'pojok', label: 'Pojok', x: 70, y: 60, accepts: 'kaktus' }] },
  'slider-puzzle':       { rows: 5, cols: 5, grid: [[1,1,1,1,1],[1,2,0,0,1],[1,1,0,1,1],[1,0,0,0,1],[1,1,1,3,1]] },
}
  ...
  if (activeMG) {
    return (
      <MiniGameHost
        ...
        onComplete={() => { setActiveMG(null) }}
        onAbandon={() => { setActiveMG(null) }}
      />
    )
  }
          ...
          <div style={{ borderTop: '1px solid #333', paddingTop: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            {MINIGAME_TYPES.map((mg) => (
              <button
                ...
                style={{ background: '#0a001a', color: '#CE93D8', border: '1px solid #555', borderRadius: 3, padding: '3px 8px', cursor: 'pointer', fontSize: 11, textAlign: 'left' }}
              >
                ▶ {mg}
              </button>
            ))}
