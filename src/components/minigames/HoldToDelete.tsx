...
export function HoldToDelete({ config, onComplete, onAbandon }: MiniGameProps<HoldToDeleteConfig>) {
  ...
  return (
    <div className="minigame-root minigame-hold-delete">
      ...
      <div className="hold-elements">
        {displayElements.map((el) => {
          ...
          return (
            <div
              ...
              tabIndex={0}
              onKeyDown={(e) => {
                if (e.key === ' ' || e.key === 'Enter') startHold(el.id)
              }}
              onKeyUp={stopHold}
            >
        ...
        {displayElements.length === 0 && <p className="mg-complete-msg">Semua terhapus.</p>}
      </div>
